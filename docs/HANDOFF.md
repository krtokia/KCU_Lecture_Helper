...
수행한다. 출석 Y가 확인되면 같은 과목을 다시 탐색하고, 해당 과목에 대상이 없을 때만
다음 과목으로 이동한다. 전 과목 완료 시에만 `PASS_ALL_COURSES_COMPLETED`로 끝낸다.

수동 pause 대기, 식별값 대조, 신호 공백 실패, 사용자 조작 중단은 기존 안전 제약을
...
탐색했고, 적격 미수강 차시는 디지털경제와 디지털금융의 이해 2주 3강, 효율적재테크와
재무관리 2주 2강·3강의 총 세 개였다.

세 대상 모두 actual playing, native ended, 10초 유예, fresh UI AJAX 응답의 해당
`atenYn: Y`를 첫 번째 검증 시도에서 확인했다. 각 완료 뒤 동일 과목 재탐색이 수행됐고,
모든 대상 처리 뒤에만 다음 과목으로 이동하거나 최종 종료했다. 추가 대상 선택, 수동
...
## 29. 2026-09-14 Navigator 1.0.0 승격 및 메뉴 정리

사용자가 실제 KCU에서 0.5.0 알림 동작에 별다른 이상이 없음을 확인했다. 남은 강의는 버그 수정
방식으로 대응하기로 하고 Navigator를 `1.0.0`으로 올렸다. 메뉴는 시작, 중지 (영상 유지),
브라우저 알림 토글, 차시 완료 알림 토글, ntfy 주소 설정/지우기, 실행 기록 저장의 6개로 줄였다.
상태 보기(콘솔 출력), 실행 기록 복사(저장의 불안정한 대체), 기록 초기화(시작·새로고침이 이미
상태를 정리함)는 함수를 보존하되 메뉴에 등록하지 않는다. 리포트 구조와 실행 로직은 바꾸지 않았다.
